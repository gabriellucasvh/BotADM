    ...
    run: async(client, interaction) => {
        if(!interaction.member.permissions.has(Discord.PermissionFlagsBits.BanMembers)) {
            ...
            member.ban({ reason: [motivo] }).then( () => {
                interaction.reply({ content: `✅ Admin: **${interaction.user.username}**, o membro ${member} (${member.id}) foi banido com sucesso.` })
            }).catch(err => {
                interaction.reply({ content: `❌ Admin: **${interaction.user.username}**, não foi possível banir o membro ${member} (${member.id}).` })
            })
        }
    }
}